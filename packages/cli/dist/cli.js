...
async function main() {
    const config = (0, cli_config_1.resolveCliConfig)(process.argv.slice(2), process.env);
    const args = config.args;
    const command = args[0];
    const client = (0, index_1.createBridgeHttpClient)(config.baseUrl);
    ...
    try {
        switch (command) {
            ...
            case "ask": {
                ...
                const askResult = await (0, index_1.waitForAskResponse)(client, text, {
            ...
            case "action": {
                ...
                if (!(0, index_1.isBridgeAction)(type)) {
                    console.error(`Error: Unknown action type: ${type}`);
                    console.error(`Supported actions: ${Object.values(index_1.BRIDGE_ACTIONS).join(", ")}`);
                    process.exit(1);
                }
            ...
            case "new-chat": {
                const result = await client.runAction(index_1.BRIDGE_ACTIONS.startNewChat);
                console.log(JSON.stringify(result, null, 2));
                break;
            }
            case "artifacts":
            case "conversations": {
                const result = await client.request(index_1.BRIDGE_PATHS.artifacts);
                console.log(JSON.stringify(result, null, 2));
                break;
            }
            case "conversation":
            ...
            case "artifact": {
                ...
                const result = await client.request(index_1.BRIDGE_PATHS.artifact(convoId, path));
                console.log(result);
                break;
            }
            default:
